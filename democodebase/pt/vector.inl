...
// Hand-unrolled partial specialization. Requires partial specialization of vector_base.
//template<typename T>
//inline vector_base<T,3> const& vector_base<T,3>::operator+=(
//    vector_base<T,3> const& other)
//{
//    T const* source = other.data();
//    m_value[0] += source[0];
//    m_value[1] += source[1];
...
template<typename T>
vector<T,2>::vector(enum uninitialized)
{
}
...
template<typename T>
vector<T,3>::vector(enum uninitialized)
{
}
...
template<typename T>
vector<T,4>::vector(enum uninitialized)
{
}
...
template<typename T>
inline T dot(vector<T,4> const& a, vector<T,4> const& b)
{
    T const* ap = a.data();
    T const* bp = b.data();
...
template<typename T>
inline T dot(vector<T,3> const& a, vector<T,3> const& b)
{
    T const* ap = a.data();
    T const* bp = b.data();
...
template<typename T>
inline vector<T,3> operator*(vector<T,3> const& v, T const& scalar)
{
    T const* v_ptr = v.data();
    return vector<T,3>(v_ptr[0] * scalar, v_ptr[1] * scalar, v_ptr[2] * scalar);
}
...
template<typename T>
vector<T,3> operator+(vector<T,3> const& a, vector<T,3> const& b)
{
    T const* a_ptr = a.data();
    T const* b_ptr = b.data();
    return vector<T,3>(
...
template<typename T>
inline T squared_length(vector<T,3> const& v)
{
    T const* v_ptr = v.data();
    return v_ptr[0] * v_ptr[0] + v_ptr[1] * v_ptr[1] + v_ptr[2] * v_ptr[2];
}
