...
float3 reflection_texcoord_offset; // default = 0
    ...
    out float4 color : COLOR0)
{
    ...
    if (length(I) * N_dot_L > 0.001)
    {
        ...
        float2 ref_texcoord = 0;
        float3 R = -reflect(V, N);
        ref_texcoord = (R.xy + 0.5) + (reflection_texcoord_offset.xy);
        float3 incoming_reflection = tex2D(reflection_sampler, ref_texcoord).xyz;
