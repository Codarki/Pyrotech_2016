...
[
    ...
    To pass information to the driver when initializing fmod use the extradriverdata parameter in FMOD_System_Init for the following reasons.
    ...
    Note! If FMOD_OUTPUTTYPE_NOSOUND_NRT or FMOD_OUTPUTTYPE_NOSOUND_NRT are used, and if the FMOD_System_Update function is being called
...
type
  ...
  (
    ...
    FMOD_OUTPUTTYPE_UNKNOWN,         (* All         - 3rd party plugin, unknown.  This is for use with FMOD_System_GetOutput only. *)
    FMOD_OUTPUTTYPE_NOSOUND,         (* All         - All calls in this mode succeed but make no sound. *)
    FMOD_OUTPUTTYPE_WAVWRITER,       (* All         - Writes output to fmodoutput.wav by default.  Use the 'extradriverdata' parameter in FMOD_System_Init, by simply passing the filename as a string, to set the wav filename. *)
    FMOD_OUTPUTTYPE_NOSOUND_NRT,     (* All         - Non-realtime version of FMOD_OUTPUTTYPE_NOSOUND.  User can drive mixer with FMOD_System_Update at whatever rate they want. *)
    FMOD_OUTPUTTYPE_WAVWRITER_NRT,   (* All         - Non-realtime version of FMOD_OUTPUTTYPE_WAVWRITER.  User can drive mixer with FMOD_System_Update at whatever rate they want. *)
...
const
    ...
    FMOD_INIT_STREAM_FROM_UPDATE       = $00000001;   (* All platforms - No stream thread is created internally.  Streams are driven from FMOD_System_Update.  Mainly used with non-realtime outputs. *)
    ...
    FMOD_INIT_PS2_DONTUSESCRATCHPAD    = $00040000;   (* PS2 only - FMOD's usage of the scratchpad between FMOD_System_Update and FMOD_System_UpdateFinished. *)
...
[
    ...
    Note!  Currently the user must call FMOD_System_Update for these callbacks to trigger!
...
*)
  tagFMOD_DSP_STATE = record
    instance: FMOD_DSP;     (* [out] Handle to the DSP hand the user created.  Not to be modified.  C++ users cast to FMOD_DSP to use.  *)
