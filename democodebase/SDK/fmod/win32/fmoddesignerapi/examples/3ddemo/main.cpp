...
FMOD::EventReverb     *reverb_waterroom   = 0;
FMOD::EventReverb     *reverb_scarycave   = 0;
...
void inWater()
{
        ...
        && yListenerPos < 5.0f)
    {
        ...
        */
        ...
        if (!reverb_waterroom)
        {
            result = fmodEventSystem->getReverb("waterroom", &reverb_waterroom);
            ERRCHECK(result);
        }
        if (reverb_waterroom)
        {
            result = reverb_waterroom->setActive(true);
            ERRCHECK(result);
    ...
    else
    {
        ...
        */
        ...
        if (!reverb_scarycave)
        {
            result = fmodEventSystem->getReverb("scarycave", &reverb_scarycave);
            ERRCHECK(result);
        }
        if (reverb_scarycave)
        {
            result = reverb_scarycave->setActive(true);
            ERRCHECK(result);
        }
...
{	
    ...
    result = fmodEventGroup->getEvent("drumloop", EVENT_DEFAULT, &objects[0].event);
    ERRCHECK(result);

    result = fmodEventGroup->getEvent("drumloop2", EVENT_DEFAULT, &objects[1].event);
    ERRCHECK(result);

    result = fmodEventGroup->getEvent("drumloop3", EVENT_DEFAULT, &objects[2].event);
    ERRCHECK(result);

    result = fmodEventGroup->getEvent("drumloop4", EVENT_DEFAULT, &objects[3].event);
    ERRCHECK(result);

    result = fmodEventGroup->getEvent("drumloop5", EVENT_DEFAULT, &objects[4].event);
    ERRCHECK(result);

    result = fmodEventGroup->getEvent("drumloop6", EVENT_DEFAULT, &objects[5].event);
    ERRCHECK(result);

    result = fmodEventGroup->getEvent("drumloop7", EVENT_DEFAULT, &objects[6].event);
    ERRCHECK(result);
...
GLuint loadTexturePNG(const char *filename)
{
    ...
    return pngBind(filename, PNG_NOMIPMAP, PNG_SOLID, &info, GL_CLAMP_TO_EDGE, GL_NEAREST, GL_NEAREST);
}
...
void init(void)
{
    ...
    ERRCHECK(result);
    result = fmodEventSystem->init(64, (FMOD_INITFLAGS)(FMOD_INIT_NORMAL | FMOD_INIT_3D_RIGHTHANDED | FMOD_INIT_OCCLUSION_LOWPASS), 0, EVENT_INIT_NORMAL);
    ERRCHECK(result);
    ...
    */
    ...
    result = fmodSystem->addDSP(global_lowpass);
    ERRCHECK(result);
