    ...
    out float4 color : COLOR0)
{
    ...
    float2 color_texcoord = texcoord + general_texcoord_offset.xy * 0.01 + color_texcoord_offset.xy;
	...
	float3 N2 = tex2D(normal_sampler, color_texcoord).xyz * 2 - 1;
	...
	float3 fake_vector1 = N;//normalize(N + V * -0.5); // High when facing camera
	float3 fake_vector2 = N2;//normalize(N + L * 0.9); // High when watched from light
	float3 fake_vector3 = N;//normalize(fake_vector1 + fake_vector2); // average
		
	//R = reflect(L,N);
	float3 R2 = reflect(L,N*0.2);
	
    float dot_fake1 = max(0, dot(V, fake_vector1));//// dot(V,R)
    float dot_fake2 = max(0, dot(V, fake_vector2));//dot(R, fake_vector2));
    float dot_fake3 = max(0, dot(V, fake_vector3));//dot(R, fake_vector3));
    ...
    final_scatter = scatter_color1 * pow(dot_fake1, 10) * I;
    final_scatter *= scatter_color2 * pow(dot_fake2, 20) * I;
	//final_scatter *= scatter_color3 * pow(dot_fake3, 10) * I;
    //final_scatter += scattering_texel3 * scatter_color3 * pow(dot_fake3, 32) * fake_I;
	final_scatter = final_scatter * 0.5 * scatter_coeff;
	//final_scatter *= 0.001;
		    
	//color.xyz = lighting;
    color.xyz = final_scatter;//* alpha_texel.r;
	//color.xyz = lighting;
    color.w = 1;
}
