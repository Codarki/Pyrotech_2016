...
void main_vs(in input_vertex input, out output_vertex output)
{
	...
	{
	    output.texcoord = input.position.yz * skaala * sign(input.normal.x);
	}
	else if (abs(input.normal.y) > 0.5)
	{
	    output.texcoord = input.position.xz * skaala * sign(input.normal.y);
	}
	else
	{
	    output.texcoord = input.position.yx * skaala * sign(input.normal.z);
	}
