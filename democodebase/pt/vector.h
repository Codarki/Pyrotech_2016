...
template<int C>
bool within_denormal(vector<float,C> const& v)
{
    for (size_t i = 0; i < C; ++i)
    {
        if (!within_denormal(v[i]))
            return false;
    }
    return true;
}

inline bool within_denormal(vector<float,3> const& v)
{
    return within_denormal(v[0]) && within_denormal(v[1]) && within_denormal(v[2]);
}
